...
while len(guesse_states) < 50:
    user_ans = screen.textinput(
        f"Guess the state({len(guesse_states)}/50)", "Whats anotoher states name?").title()
    if user_ans == "Exit":
        break
...
not_guessed = pd.DataFrame(
    list(set(all_states)-set(guesse_states))).to_csv("states to learn")
